function varargout = Noshir_GUI(varargin)
% NOSHIR_GUI MATLAB code for Noshir_GUI.fig
%      NOSHIR_GUI, by itself, creates a new NOSHIR_GUI or raises the existing
%      singleton*.
%
%      H = NOSHIR_GUI returns the handle to a new NOSHIR_GUI or the handle to
%      the existing singleton*.
%
%      NOSHIR_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NOSHIR_GUI.M with the given input arguments.
%
%      NOSHIR_GUI('Property','Value',...) creates a new NOSHIR_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Noshir_GUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Noshir_GUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Noshir_GUI

% Last Modified by GUIDE v2.5 13-Oct-2021 16:53:51

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Noshir_GUI_OpeningFcn, ...
                   'gui_OutputFcn',  @Noshir_GUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Noshir_GUI is made visible.
function Noshir_GUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Noshir_GUI (see VARARGIN)

% Choose default command line output for Noshir_GUI
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Noshir_GUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Noshir_GUI_OutputFcn(hObject, eventdata, handles) 
...
axes(handles.axes3);
imshow(img)
 set(handles.axes3,'UserData',img);
...
img=get(handles.axes3,'UserData');
img_Veq=hsveq(img);
axes(handles.axes1);
imshow(img_Veq);
set(handles.axes1,'UserData',img_Veq);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
img_Veq=get(handles.axes1,'UserData');
img=get(handles.axes3,'UserData');
scale=1;
[img_r,img_g,img_b,img,nrows,ncols] = prepare(img,scale); 
hsv=rgb2hsv(img_Veq);
[nrows ncols]=size(hsv(:,:,1));
hs = double(hsv(:,:,1:2));
...
axes(handles.axes4);
imshow(img_bound_nuc)
set(handles.axes4,'UserData',img_nuc);
...
NH1 =10;                % NH1 = Number of neurons in the first hidden layer
No = size(groups,1);    % No = Number of neurons in the output layer
...
%    net = newff(minmax(TrainingSet),[NH1 No],{'logsig' 'logsig'},'trainrp');
%  
%    [net,tr] = train(net,TrainingSet,GroupTrain);
load('net_BPNN')
   pred  = sim(net, TestSet);
   [~,indnn] = max(pred);
 
% figure(6);plotfit(net,TrainingSet,GroupTrain);
axes(handles.axes2);
imshow(img);
[bw_label,n]=bwlabel(bw);
